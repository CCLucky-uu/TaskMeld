...
const envelopeToJson = (overrides?: Partial<ResultEnvelope>): string =>
  JSON.stringify(makeEnvelope(overrides));
...
const run = async () => {
  ...
  // 1. Text without envelope markers returns no candidates
  {
    ...
    assert.equal(
      result.length,
      0,
      "text without envelope markers should return no candidates",
    );
  }
  ...
  // 2. Text with all three markers is scanned and envelope is found
  {
    ...
    assert.equal(
      result.length,
      1,
      "text with all three markers should be scanned",
    );
  ...
  // 3. Partial markers: has "version" and "requestId" but no "artifacts"
  {
    ...
    assert.equal(
      result.length,
      0,
      "text with only partial markers should be skipped",
    );
  }
  ...
  //    extraction path that handles envelopes embedded in large prefixed texts.
  {
    ...
    assert.ok(
      text.length <= 60_000,
      "text must be <= 60K to pass extractTextCandidates filter",
    );
    const result = collectEnvelopeCandidates({ text });
    assert.equal(
      result.length,
      1,
      "envelope at end of large text should be found",
    );
    assert.equal(result[0].requestId, "req-test-1");
  }
  ...
  // 5. Large text (over 60K, no markers) is safely filtered — no errors
  {
    ...
    assert.equal(
      result.length,
      0,
      "large text without envelope markers should return no candidates",
    );
  }
  ...
  // 6. Envelope at the very beginning of text (no prefix)
  {
    ...
    assert.equal(
      result.length,
      1,
      "envelope at start of text should be found",
    );
    assert.equal(result[0].requestId, "req-test-1");
  }
  ...
  // 7. Envelope inside a code fence (```json ... ```)
  {
    ...
    assert.equal(
      result.length,
      1,
      "envelope inside code fence should be parsed",
    );
    assert.equal(result[0].requestId, "req-test-1");
  }
  ...
  // 9. Long content truncated with [truncated] suffix
  {
    ...
    assert.ok(
      ...
      "truncated content should carry [truncated] suffix",
    );
    // Verify the truncated portion is exactly the first 8000 chars + suffix
    assert.ok(
      result.startsWith("a".repeat(8_000)),
      "truncated result should start with first 8000 chars",
    );
  }
  ...
  // Also test content exactly at the limit
  {
    ...
    assert.equal(
      result,
      exactText,
      "content at exactly 8000 chars should not be truncated",
    );
  }
  ...
  {
    assert.equal(
      detectFenceLanguage('{"key": "value"}'),
      "json",
      "valid JSON object should return json",
    );
    assert.equal(
      detectFenceLanguage("[1, 2, 3]"),
      "json",
      "valid JSON array should return json",
    );
  }
  ...
  {
    assert.equal(
      detectFenceLanguage("hello world"),
      "text",
      "plain text should return text",
    );
    assert.equal(
      detectFenceLanguage("{invalid json"),
      "text",
      "malformed JSON should return text",
    );
  }
  ...
  // 13. Empty / whitespace-only strings
  {
    assert.equal(
      detectFenceLanguage(""),
      "text",
      "empty string should return text",
    );
    assert.equal(
      detectFenceLanguage("   "),
      "text",
      "whitespace-only string should return text",
    );
  }
