...
} from "../src/pipeline/execution/dependency-check"
...
const makeRouteEnvelope = (nodeId: string, routes: string[]): ResultEnvelope => ({
  ...
  status: "success",
  artifacts: [
    { type: "route.v1", schemaVersion: 1, name: "route", content: routes.map((r) => ({ route: r })), meta: {} },
  ],
...
const run = async () => {
  ...
  // blocks B2 because the route doesn't match.
  {
    const workflow: WorkflowDefinitionRuntime = {
      ...
      scheduler: {
        ...
        dispatchBy: "item",
        maxConcurrency: 1,
        loopGuard: { maxGlobalIterations: 20, maxPerItemLoop: 5 },
      },
      plugins: [
        {
  ...
  // ====== Test 8: Route derived itemKey can execute independently from mainline ======
  {
    ...
    assert.equal(
      noBase?.status,
      "skipped",
      "base item should be skipped after dep check (route edge from base router not satisfied)",
    )
    ...
    assert.notEqual(
      afterNoBase?.status,
      "queued",
      "after-no on base key should NOT be queued — only derived key path executes",
    )
  }
