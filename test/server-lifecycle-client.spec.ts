...
const { createServerLifecycleClient } = require("../src/cli/server-runtime-client") as typeof import("../src/cli/server-runtime-client");
  ...
  try {
...

    {
      ...
      try {
        ...
        await assert.rejects(
          readFile(startupLockPath, "utf8"),
          /ENOENT/,
          "恢复完成后 startup lock 应被释放",
        );
      } finally {
        restoreFetch();
      }
