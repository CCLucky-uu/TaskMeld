...
} from "../src/artifacts/storage-service"
...
const run = async () => {
  // ===== buildArtifactStorageDirs =====
  {
    ...
    assert.ok(
      dirs.runDir.endsWith(join("success", b, "run-001")),
      `runDir should include status/date/runId: ${dirs.runDir}`,
    )
...

  {
    ...
    assert.equal(dirs.dateBucket, b)
    assert.ok(
      dirs.runDir.includes(join("rejected", b, "run-003")),
      "rejected status should use same date/runId structure",
    )
  }
  ...
  try {
    ...
    assert.ok(
      ...
      `should find at least 3 artifacts (new rejected + old rejected + success), got ${allArtifacts.length}`,
    )

    // 新 rejected 产物可查
    const newRejectedItem = allArtifacts.find(
      (a: StoredArtifactItem) => a.status === "rejected" && a.runId === "run-rej-001",
    )
    ...
    if (newRejectedItem) {
      ...
      assert.ok(
        newRejectedItem.relativePath.includes("rejected/2026-05-17/run-rej-001"),
        `new rejected should be in structured path, got: ${newRejectedItem.relativePath}`,
      )
    }

    // 旧 rejected 扁平文件兼容 — 即使 runId 解析可能失败也应出现在列表中
    const oldRejectedItem = allArtifacts.find(
      (a: StoredArtifactItem) => a.status === "rejected" && a.relativePath.toLowerCase().includes("legacy"),
    )
    ...
    const successEnvelopeItem = allArtifacts.find((a) => a.relativePath.toLowerCase().includes("/envelopes/"))
    ...
    const newFormatItem = allArtifacts.find((a) => a.relativePath.toLowerCase().includes("n3-artifact"))
    ...
    assert.ok(
      pastFiltered.items.length >= 2,
      `date range should include both structured artifacts, got ${pastFiltered.items.length}`,
    )
