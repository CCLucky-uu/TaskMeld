...
const makeRouteEnvelope = (
  nodeId: string,
  routes: string[],
): import("../src/pipeline/structured-output/contract").ResultEnvelope => ({
  version: "2.0",
  runId: "run-test",
  nodeId,
  requestId: "req-1",
...
const run = async () => {
  ...
  // ====== Test 5: Retry route source node clears old derivations ======
  {
    ...
    const allDerived = (store.getRun().itemRuns ?? []).filter((item) => item.itemKey.startsWith("kw-1::router:"))
