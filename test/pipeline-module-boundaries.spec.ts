...
const parseRelativeImports = (filePath: string, allPipelineFiles: Set<string>): string[] => {
  ...
  const importRe = /(?:import|export)\s+(?:type\s+)?(?:\{[^}]*\}|\*\s+as\s+\w+|\w+)\s*(?:,\s*(?:\{[^}]*\}|\*\s+as\s+\w+))?\s*from\s*['"](\.\.[/\\][^'"]+|[.][/\\][^'"]+)['"]/g;
  let match;
...
const run = async () => {
  ...
  if (cycles.length > 0) {
    ...
    const unknownCycles = cycleDescriptions.filter(
      (desc) => !knownCycles.some((known) => desc === known || desc.includes(known.slice(0, -known.length + known.indexOf(" → ") + 3))),
    );
    ...
    assert.equal(unknownCycles.length, 0, `Unexpected import cycles found:\n${unknownCycles.join("\n")}\n\nKnown cycles (to be fixed in Phase 1):\n${knownCycles.join("\n")}`);
  }
  ...
  if (allFilesSet.has(typesFile)) {
    ...
    assert.equal(nonExternal.length, 0, `types/workflow.ts should not import from pipeline:\n${nonExternal.map(rel).join("\n")}`);
  }
