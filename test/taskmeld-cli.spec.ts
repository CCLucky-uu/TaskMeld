...
const { createPipelineRuntimeApiClientWs: createPipelineRuntimeApiClient, createServerLifecycleClient } =
        ...
        listAgents: async () => [
          {
            ...
            raw: {
              ...
              model: {
                ...
                fallbacks: ["xiaomi/mimo-v2-pro", "minimax/MiniMax-M2.7", "xiaomi/mimo-v2.5", "xiaomi/mimo-v2.5-pro"],
              },
            },
...
const run = async () => {
...

  {
    ...
    assert.match(
      md,
      /\| Agent ID \| Name \| Workspace \| Runtime \| Model Primary \| Last Active At \|/m,
      "agent list --format md 表头应包含扩展字段",
    )
  }
...

  {
    ...
    const payload = JSON.parse(out.read()) as {
      ok?: boolean
      command?: string
      data?: { basic?: Record<string, unknown> }
...

  {
    ...
    try {
      ...
      const rewritten = JSON.parse(await readFile(serverRuntimeMetadataPath, "utf8")) as {
        serverId?: string
        pid?: number
      }
      ...
      app: {
        ...
        pipelineService: {
          ...
          startPipeline: async () => ({
            ...
            pipelineId: "A",
            accepted: true,
            batchRunId: "batch:A:2026-05-08T00:00:00.000Z",
          }),
...

  {
    ...
    assert.match(
      md,
      /Last completed at: 2026-05-08T00:00:00.000Z/m,
      "pipeline status markdown 非运行态应显示 lastCompletedAt",
    )
...

  {
    ...
    const payload2 = JSON.parse(out.read()) as {
      ok?: boolean
      command?: string
      data?: { basic?: Record<string, unknown> }
      basic?: Record<string, unknown>
      ...
      app: {
        ...
        pipelineService: {
          ...
          startPipeline: async (pipelineId: string) => ({
            ...
            accepted: true,
            runId: "run-compat",
            run: { id: "run-compat", status: "running" },
          }),
          getPipelineStatus: async () => ({
            ok: true,
            pipelineId: "A",
    ...
    const stopSelectorBootstrap: CliBootstrap = async () => ({
      app: {
        ...
        pipelineService: {
          ...
          getPipelineStatus: async () => ({
            ok: true,
            status: { pipelineId: "A", running: true, batchRun: { status: "running" } },
          }),
