...
const run = async () => {
  ...
  // ====== 8. Primitive detail passes through ======
  {
    ...
    assert.equal(sanitizeDiagnosticPayload(LONG), LONG, "top-level string not a target field value, should pass through");
  ...
  // ====== Edge: Error-like object with long message ======
  {
    ...
    assert.ok(typeof result.stack === "string" && (result.stack as string).length > 2048, "stack should not be truncated");
  }
