type TaskMeldIconProps = {
  className?: string;
};

export function TaskMeldIcon({ className }: TaskMeldIconProps) {
  return (
    <svg
      viewBox="0 0 76.05 83.85"
      aria-hidden="true"
      className={className}
      fill="currentColor"
    >
      {/* 使用 currentColor 继承项目主题色，避免图标颜色与主题割裂。 */}
      <path d="M12.29,64.28c-1.12,0-2.24-.47-3.03-1.38C3.29,56,0,47.17,0,38.03,0,17.06,17.06,0,38.03,0c5.29,0,10.41,1.07,15.21,3.17,2.02.88,2.95,3.24,2.06,5.27-.89,2.03-3.24,2.95-5.27,2.06-3.79-1.66-7.83-2.5-12.01-2.5-16.56,0-30.03,13.47-30.03,30.03,0,7.22,2.6,14.19,7.31,19.64,1.45,1.67,1.26,4.2-.41,5.64-.76.66-1.69.98-2.62.98Z" />
      <path d="M63.76,64.29c-.93,0-1.86-.32-2.62-.98-1.67-1.45-1.85-3.97-.41-5.64,4.72-5.45,7.31-12.42,7.31-19.64,0-2.86-.4-5.69-1.19-8.4-.62-2.12.6-4.34,2.72-4.96,2.12-.62,4.34.6,4.96,2.72,1,3.44,1.51,7.02,1.51,10.64,0,9.14-3.29,17.97-9.26,24.88-.79.91-1.91,1.38-3.03,1.38Z" />
      <path d="M42.14,69.28c0,1.54-.85,2.89-2.11,3.59v8.98c0,1.1-.9,2-2,2s-2-.9-2-2v-8.98c-1.26-.7-2.11-2.05-2.11-3.59s.85-2.89,2.11-3.59v-9.13c0-1.1.9-2,2-2s2,.9,2,2v9.13c1.26.7,2.11,2.05,2.11,3.59Z" />
      <path d="M54.61,66.83c0,1.54-.85,2.89-2.11,3.59v4.87c0,1.11-.89,2-2,2s-2-.89-2-2v-4.88c-1.25-.7-2.1-2.04-2.1-3.58s.85-2.88,2.1-3.58v-9.77c0-1.11.9-2,2-2s2,.89,2,2v9.76c1.26.7,2.11,2.05,2.11,3.59Z" />
      <path d="M28.61,66.83c0,1.54-.85,2.89-2.11,3.59v4.87c0,1.11-.9,2-2,2s-2-.89-2-2v-4.87c-1.26-.7-2.11-2.05-2.11-3.59s.85-2.89,2.11-3.59v-9.76c0-1.11.9-2,2-2s2,.89,2,2v9.76c1.26.7,2.11,2.05,2.11,3.59Z" />
      <circle cx="64.46" cy="15.07" r="5.46" />
      <path d="M48.51,23.51h-20.96c-3.11,0-5.92,1.26-7.96,3.3-2.04,2.03-3.3,4.84-3.3,7.95,0,6.22,5.04,11.26,11.26,11.26h20.96c3.1,0,5.92-1.26,7.95-3.3,2.04-2.03,3.3-4.85,3.3-7.96,0-6.21-5.04-11.25-11.25-11.25ZM27.82,39.56c-2.65,0-4.79-2.15-4.79-4.8s2.14-4.79,4.79-4.79,4.79,2.15,4.79,4.79-2.14,4.8-4.79,4.8ZM48.84,39.56c-2.65,0-4.8-2.15-4.8-4.8s2.15-4.79,4.8-4.79,4.79,2.15,4.79,4.79-2.15,4.8-4.79,4.8Z" />
    </svg>
  );
}
