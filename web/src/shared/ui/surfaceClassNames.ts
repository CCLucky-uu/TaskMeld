/* All form controls derive their styles from here.
   Business components may only add size/layout on the outside — never individually override color, border, hover, or focus.
   Otherwise the same input would grow three distinct looks across agents, pipelines, and modals, making maintenance scatter. */
...
// When an input sits inside a nested container, darken it one step further to avoid blending with the container background.
...
/** Form control error-state override: append after controlInputClassName to swap border and background */
