...
export async function fetchSessions(): Promise<SessionItem[]> {
  const data = await requestJson<SessionListResponse>("/api/sessions");
...
export async function sendSessionMessage(params: {
  ...
  return requestJson<SessionSendResponse>(`/api/sessions/${encodeURIComponent(params.sessionId)}/send`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export async function fetchSessionHistory(params: {
  ...
  const query = typeof params.limit === "number" ? `?limit=${Math.max(1, Math.floor(params.limit))}` : "";
  const data = await requestJson<SessionHistoryResponse>(
    `/api/sessions/${encodeURIComponent(params.sessionId)}/history${query}`,
  );
