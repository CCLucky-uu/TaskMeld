...
export function ArtifactPreviewPane({
  ...
  return (
    <section className="grid min-h-0 grid-rows-[auto_auto_minmax(0,1fr)] border border-(--line) bg-[rgba(12,21,27,0.85)]">
      <div className="border-b border-(--line) px-2 py-1.5 text-xs text-(--muted)">产物详情</div>
      ...
      <div className="min-h-0 overflow-auto p-2">
        {!item ? <p className={`${monoClassName} m-0 text-xs text-(--muted)`}>请先在左侧选择一个产物文件</p> : null}
        {item && isLoading ? <p className={`${monoClassName} m-0 text-xs text-(--muted)`}>内容加载中...</p> : null}
        {item && !isLoading && contentError ? <p className={`${monoClassName} m-0 text-xs text-(--bad)`}>内容加载失败: {contentError}</p> : null}
        {item && !isLoading ? (
