...
export function ArtifactTreePane({ groups, selectedItemKey, loading, error, onSelect }: ArtifactTreePaneProps) {
  ...
  return (
    <section className="grid min-h-0 grid-rows-[auto_minmax(0,1fr)] border border-(--line) bg-[rgba(12,21,27,0.85)]">
      ...
      <div className="min-h-0 overflow-auto px-2 py-1.5">
        ...
        {!loading && error ? (
          <p className={`${monoClassName} m-0 text-xs text-(--bad)`}>{t("loadFailed", { error })}</p>
        ) : null}
