...
import {
  isNodeReturnErrorNode,
  isStructuredErrorNode,
  parsePipelineError,
} from "../../../entities/pipeline/error-display";
      ...
      <div className="p-3">
        <div className="grid grid-cols-[repeat(auto-fill,minmax(310px,1fr))] gap-3">
          {pipelines.map((pipeline) => {
            ...
            const finishedCount = pipeline.nodes.filter((node) =>
              ["success", "failed", "skipped", "stopped"].includes(node.status),
            ).length;
            ...
            return (
              <article key={pipeline.id} className={cardBaseClassName}>
                ...
                <div className="grid content-start gap-1">
                  ...
                  <p className={`${monoClassName} m-0 truncate text-sm text-(--text)`}>
                    {latestArtifact?.name ?? t("noArtifact")}
                  </p>
