      ...
      >
        ...
        <div className="mb-3 space-y-2 text-xs">
          <div className="flex gap-2">
            ...
            <span className={`font-mono ${isDestructive ? 'text-[var(--bad)]' : 'text-[var(--live)]'}`}>{toolName}</span>
          </div>
          <div className="flex gap-2">
            <span className="shrink-0 text-[var(--muted)]">Arguments:</span>
            <pre className="m-0 max-h-[120px] overflow-auto whitespace-pre-wrap break-all font-mono text-[var(--text)]">{JSON.stringify(toolArgs, null, 2)}</pre>
          </div>
          {isDestructive && (
