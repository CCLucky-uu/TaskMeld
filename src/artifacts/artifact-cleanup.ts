 ...
 */
export const cleanupEmptyDirs = async (artifactDir: string): Promise<{ removed: number; warnings: string[] }> => {
  const warnings: string[] = []
  let removed = 0
  ...
  const walkAndRemove = async (dirPath: string): Promise<void> => {
    try {
      ...
      const isRoot =
        dirPath === artifactDir ||
        (dirPath.endsWith(`${sep}artifacts`) && dirPath.replace(/\\/g, "/").endsWith("/artifacts"))
      if (isRoot) return
 ...
 */
export const cleanupTempFiles = async (artifactDir: string): Promise<{ cleaned: number; warnings: string[] }> => {
  const warnings: string[] = []
  let cleaned = 0
