    ...
    for (const filePath of files) {
      ...
      const effectiveRunId =
        ...
        (meta.status === "rejected"
          ? (() => {
              const m = fileName.match(/^(run-.+?)(?:-n\d|-rejected-by)/)
              return m ? m[1] : null
            })()
          : null)
      ...
      if (kindFilter) {
        const scanKind = meta.relativePath.includes("/envelopes/")
          ? "envelope"
          : fileName.includes("-adapter-output")
            ? "adapter"
            : fileName.includes("-group-output")
              ? "group"
              : "artifact"
      ...
      if (dateFrom !== null || dateTo !== null) {
        const rowDateBucket = DATE_BUCKET_RE.test(meta.dateBucket)
          ? meta.dateBucket
          : formatDateBucketFromIso(updatedAt)
...
export const listStoredArtifacts = async (
  ...
  const limit = Number.isFinite(options?.limit)
    ? Math.max(1, Math.min(5000, Math.trunc(options?.limit as number)))
    : 100
...
export const readStoredArtifactContent = async (
  ...
  const content = (() => {
    ...
    if (envelopeObj) {
      ...
      const contents = artifacts.map((item) => toRecord(item)?.content).filter((item) => item !== undefined)
