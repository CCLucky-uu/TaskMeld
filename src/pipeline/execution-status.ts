...
const readCurrentBatchProgress = (
  run: PipelineExecutionRunLike,
  batchRun: unknown,
): PipelineCurrentBatchProgress | null => {
  const batchRecord = batchRun && typeof batchRun === "object" ? (batchRun as Record<string, unknown>) : null
  const currentBatchItemKey = isNonEmptyString(batchRecord?.currentBatchItemKey)
    ? batchRecord.currentBatchItemKey.trim()
    : null
  ...
  const runningNodeIds = normalizeIdList(
    currentItemRuns.filter((item) => item.status === "running").map((item) => String(item.nodeId ?? "").trim()),
  )
  ...
  const runningGroupIds = normalizeIdList(
    currentGroupItemRuns.filter((item) => item.status === "running").map((item) => String(item.groupId ?? "").trim()),
  )
