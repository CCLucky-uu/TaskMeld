...
const buildIndices = (workflow: WorkflowDefinitionRuntime): WorkflowIndices => {
  ...
  const allEntities = [...workflow.nodes.map((n) => ({ id: n.id })), ...workflow.groups.map((g) => ({ id: g.id }))]
...
export const createWorkflowGraph = (
  initialWorkflow: WorkflowDefinitionRuntime,
  initialTemplateNodes?: PipelineTemplateNode[],
) => {
