...
export const shouldFailFastForCompletedSession = (sessionCompletedAt: number | null, now = Date.now()) =>
...
export const evaluateEnvelopeCandidates = (
  ...
  const related = envelopes.filter(
    (envelope) =>
      envelope.runId === ctx.runId && envelope.nodeId === ctx.nodeId && envelope.sessionId === ctx.sessionId,
  )
  if (related.length > 0) {
