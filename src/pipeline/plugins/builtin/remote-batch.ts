...
export const remoteBatchPlugin: PipelinePlugin<RemoteBatchConfig> = {
  ...
  hooks: {
    ...
    onBatchStart(ctx) {
      console.log(
        `[plugin:remote-batch] pipeline=${ctx.pipelineId} batch=${ctx.batchIndex + 1}/${ctx.totalBatches} items=${ctx.batchItems.length}`,
      )
    },

    onBatchComplete(ctx) {
      console.log(
        `[plugin:remote-batch] pipeline=${ctx.pipelineId} batch=${ctx.batchIndex + 1} complete, total processed: ${ctx.totalItems}`,
      )
    },

    onBatchError(ctx, error) {
      console.error(
        `[plugin:remote-batch] pipeline=${ctx.pipelineId} batch=${ctx.batchIndex + 1} error:`,
        error.message,
      )
    },
  },
