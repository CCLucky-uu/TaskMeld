...
export const normalizeWorkflowPlugins = (value: unknown): WorkflowPlugins => {
  ...
  if (Array.isArray(value)) {
    return value.map((item) => normalizePluginInstance(item)).filter((x): x is PluginInstance => x !== null)
  }
  ...
  return [{ pluginId: "scheduler", enabled: false, config: {} }]
}
...
export const readWorkflowDefinitionFromRawDetailed = (value: unknown): WorkflowReadResult => {
  ...
  if (value.version !== "3.0") {
    return {
      ...
      detail: `Invalid workflow.version: ${String(value.version ?? "")}`,
    }
  }
