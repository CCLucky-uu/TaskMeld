...
export const pickDefaultRejectTarget = (node: NodeRun, nodes: NodeRun[], dependencyIds?: string[]): string | null => {
...
export const handleNodeReject = async (params: {
  ...
  if (node.rejectCount > limit) {
    markNodeFailed(
      node,
      ctx("reject_limit_exceeded", { error: JSON.stringify(envelope.error ?? "reject_limit_exceeded") }),
    )
  ...
  const rejectTargetIds =
    explicitTargets.length > 0
      ...
      : defaultTarget
        ...
        : []
  const rejectMessage =
    extractEnvelopeErrorMessage(envelope.error) || "Downstream validation failed, please correct and resubmit."

  if (rejectTargetIds.length === 0) {
    markNodeFailed(
      node,
      ctx("reject_target_missing", { error: JSON.stringify(envelope.error ?? "reject_target_missing") }),
    )
