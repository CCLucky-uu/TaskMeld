...
import { type ArtifactManifest, type GroupItemRun, type NodeItemRun, type NodeRun } from "../runtime-model"
...
export const createExecutionService = (deps: ExecutionServiceDeps) => {
  ...
  const resetAffectedDownstreamNodes = (params: {
    ...
    for (const affectedGroupItem of run.groupItemRuns ?? []) {
      ...
      markGroupItemReset(
        affectedGroupItem,
        computeInitialGroupItemStatus(affectedGroupItem.groupId),
