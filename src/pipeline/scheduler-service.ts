...
export const createSchedulerService = (deps: SchedulerServiceDeps) => {
  ...
  const pickNextRunnableBatch = (
    maxBatchSize = Math.max(1, deps.graph.getWorkflow().scheduler.maxConcurrency || 1),
  ) => {
  ...
  const retryNodeExecution = async (nodeId: string, itemKey?: string) => {
    ...
    for (const currentGroupItem of getRun().groupItemRuns ?? []) {
      ...
      markGroupItemReset(currentGroupItem, state.computeInitialGroupItemStatus(currentGroupItem.groupId), {
        reason: "retry_reset",
      })
  ...
  const drainPipeline = async (reason: string, signal?: AbortSignal) => {
    ...
    if (drainInFlight) {
      deps.runtimeStore.pushTimeline(
        `[Scheduling drain lock] caller=${reason} triggered drain, but ${drainInFlightReason} drain is already in progress, merged into existing drain`,
        "info",
      )
    ...
    drainInFlight = (async () => {
      ...
      const launchItem = (item: GroupItemRun | NodeItemRun) => {
        const task = (
          "nodeId" in item ? deps.executionService.executeNodeItem(item) : deps.executionService.executeGroupItem(item)
        )
      ...
      while (true) {
        ...
        while (!stopScheduling && !signal?.aborted && active.size < maxConcurrency && executed < maxIterations) {
          ...
          const batchLabel = batch
            .map((item) =>
              "nodeId" in item ? `${item.nodeId}#${item.itemKey}` : `group:${item.groupId}#${item.itemKey}`,
            )
            .join(", ")
  ...
  const itemBatchController = createItemBatchController({
    ...
    executeBatch: async ({ batchItems, batchIndex, totalBatches, totalItems }) => {
      ...
      deps.runtimeStore.emitPipeline()
      deps.runtimeStore.pushTimeline(
        `Batch run started: batch ${batchIndex}/${totalBatches}, keywords ${batchItems.length}/${totalItems}`,
      )
      ...
      if (getRun().status === "running") {
        deps.runtimeStore.pushTimeline(
          `Batch run ended: batch ${batchIndex} has no further executable nodes, proceeding to next batch`,
          "warn",
          {
            batchIndex,
            totalBatches,
  ...
  return {
    ...
    startBatchRun: (items: string[], batchSize?: number, options?: { startIndex?: number }) => {
      ...
      if (started.ok) {
        deps.runtimeStore.pushTimeline(
          `Batch run started: ${started.snapshot.totalItems} total items, ${started.snapshot.batchSize} per batch`,
        )
      }
      return started
    },
