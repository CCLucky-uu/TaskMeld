...
const scanRunLogLines = async (logFile: string, onLine: (line: string) => void) => {
...
export const readRunLogPage = async (logFile: string, query: RunLogQuery): Promise<RunLogPage> => {
  ...
  const start = order === "asc" ? offset : Math.max(0, total - offset - limit)
  const end = order === "asc" ? Math.min(total, offset + limit) : Math.max(0, total - offset)
