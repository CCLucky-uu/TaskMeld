...
import { createGatewaySkillService } from "./services/gateway-skill-service"
...
if (require.main === module) {
  ;(async () => {
    ...
    const gatewaySkill = createGatewaySkillService(app)
    const wevraPluginRegistry = createBuiltinPluginRegistry()
    const wevraAgent = new WevraAgent(undefined, wevraServices, gatewaySkill, app, wevraPluginRegistry)
    initWevraWs(wevraAgent, wsBroker)
    await wevraAgent.init() // 恢复对话
    console.log(
      `wevra-ready  model=${wevraAgent.getStatus().model}  tools=${wevraAgent.toolRegistry.size}  conversations=${wevraAgent.getStatus().conversations}`,
    )
    ...
    apiServer.listen(appContext.api.port, appContext.api.host, () => {
      ...
      console.log(
        `api-server-ready v${process.env.npm_package_version ?? "?.?.?"}  http://${appContext.api.host}:${appContext.api.port}`,
      )
    })
    ...
    if (appContext.gateway.url && appContext.gateway.token) {
      appContext.gateway
        ...
        .then((hello) => {
          ...
          console.log(
            `gateway-ready  server=${sv?.version ?? "?"}  conn=${sv?.connId ?? "?"}  proto=v${(hello as any)?.protocol ?? "?"}`,
          )
        ...
        .catch((error) => {
          ...
          console.log(
            `taskmeld v${process.env.npm_package_version ?? "?.?.?"}  running (gateway disconnected), Ctrl+C to stop`,
          )
          process.exitCode = 1
        })
    } else {
