...
const waitForPipelineWatchSignal = async (
  ...
  const eventType = await new Promise<"bootstrap" | "pipeline.updated">((resolve, reject) => {
    ...
    const matchesBootstrap = (payload: Record<string, unknown>): boolean => {
      const pipelines =
        payload.pipelines && typeof payload.pipelines === "object"
      ...
      if (selector.runId) {
        for (const item of Object.values(pipelines)) {
          ...
          const runId =
            typeof (item as { runId?: unknown }).runId === "string" ? String((item as { runId?: unknown }).runId) : ""
    ...
    ws.on("open", () => {
      timeoutHandle = setTimeout(() => {
        finish(
          new CliError("Pipeline watch event-stream wait timed out", {
    ...
    ws.on("error", () => {
      finish(
        new CliError("Pipeline watch event-stream unavailable", {
    ...
    ws.on("close", () => {
      if (!settled) {
        finish(
          new CliError("Pipeline watch event-stream closed", {
...
const metadataMatchesHealth = (metadata: ServerRuntimeMetadata | null, health: ServerHealthPayload): boolean => {
...
const reconcileRuntimeMetadataWithHealth = async (
  probe: RuntimeOwnershipProbe,
): Promise<ServerRuntimeMetadata | null> => {
...
export const createPipelineRuntimeApiClientWs = () => {
  ...
  return {
    ensureServerReady,
    startPipeline: async (pipelineId: string) => wsClient.sendReq("pipeline.run", { pipelineId }),
    ...
    listOutputs: async (pipelineId: string) => wsClient.sendReq("pipeline.output.list", { pipelineId }),
    listLinks: async () => wsClient.sendReq("pipeline.link.list"),
    getQueue: async (pipelineId: string) => wsClient.sendReq("pipeline.queue.list", { pipelineId }),
  }
}
