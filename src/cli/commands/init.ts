...
export const initCommand: CliCommandHandler = async (input) => {
  ...
  if (interactive) {
    ...
    if (!url) {
      hr()
      url =
        (await fieldPrompt(rl, t("init:gatewayUrlLabel"), t("init:gatewayUrlHint"), "ws://127.0.0.1:18789")) ||
        "ws://127.0.0.1:18789"
    }
...
export const initRoutes: CliRouteDefinition[] = [
  {
    ...
    help: {
      ...
      examples: [t("init:example1"), t("init:example2")],
      notes: [t("init:note1"), t("init:note2"), t("init:note3")],
    },
  },
]
