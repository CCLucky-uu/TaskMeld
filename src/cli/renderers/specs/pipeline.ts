    ...
    rows: (data) => {
      ...
      return [
        ...
        },
        {
          field: "Running Nodes",
          value:
            asArray(currentBatch.runningNodeIds)
              .map((item) => pickText(item))
              .join(", ") || "-",
        },
        {
          field: "Pending Nodes",
          value:
            asArray(currentBatch.pendingNodeIds)
              .map((item) => pickText(item))
              .join(", ") || "-",
        },
        {
          field: "Completed Nodes",
          value:
            asArray(currentBatch.completedNodeIds)
              .map((item) => pickText(item))
              .join(", ") || "-",
        },
        {
          field: "Failed Nodes",
          value:
            asArray(currentBatch.failedNodeIds)
              .map((item) => pickText(item))
              .join(", ") || "-",
        },
      ]
    ...
    sections: [
      ...
      },
      {
        ...
        rows: (data) => {
          ...
          const downstreamByNode = new Map<string, string[]>()
          const edges = isBatch ? asArray(d.edges) : asArray(readRecord(readRecord(data).workflow).edges)
          for (const edge of edges) {
            const edgeRec = readRecord(edge)
            const from = pickText(edgeRec.from)
            const to = pickText(edgeRec.to)
    ...
    sections: [
      ...
      },
      {
        ...
        rows: (data) => {
          ...
          const activeIds = asArray(status.activeNodeIds)
            .map((n) => pickText(n))
            .filter((n) => n !== "-")
          const pendingIds = asArray(status.pendingNodeIds)
            .map((n) => pickText(n))
            .filter((n) => n !== "-")
          return [
            { field: "Run Status", value: status.runStatus },
            ...
            if (batches.length === 0) {
              ...
              for (const batch of batches) {
                ...
                const activeNodes = batchNodes.filter((n) => {
                  const hasContent =
                    (Array.isArray(n.content) && n.content.length > 0) || (typeof n.content === "string" && n.content)
