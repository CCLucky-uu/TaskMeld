...
export const registerAgentWsMethods = (registry: WsMethodRegistry): void => {
  ...
  registry.register("agent.create", async (params, ctx) => {
    try {
      ...
      const payload = await ctx.services.client.sendReq("agents.create", { name, workspace })
  ...
  registry.register("agent.update", async (params, ctx) => {
    try {
      ...
      if (typeof params.workspace === "string" && params.workspace.trim())
        updateParams.workspace = params.workspace.trim()
      const payload = await ctx.services.client.sendReq("agents.update", updateParams)
      return { ok: true, payload: payload ?? {} }
    } catch (error) {
      return { ok: false, error: formatError(error) }
    }
  ...
  registry.register("agent.delete", async (params, ctx) => {
    try {
      ...
      const payload = await ctx.services.client.sendReq("agents.delete", deleteParams)
      return { ok: true, payload: payload ?? {} }
    } catch (error) {
      return { ok: false, error: formatError(error) }
    }
