...
export const registerPipelineLinksWsMethods = (registry: WsMethodRegistry): void => {
  ...
  registry.register("pipeline.link.create", async (params, ctx) => {
    ...
    const contract =
      params.inputContract && typeof params.inputContract === "object"
        ? {
            requireType:
              typeof (params.inputContract as Record<string, unknown>).requireType === "string"
                ? ((params.inputContract as Record<string, unknown>).requireType as string)
                : undefined,
            requireSchemaVersion:
              typeof (params.inputContract as Record<string, unknown>).requireSchemaVersion === "number"
                ? ((params.inputContract as Record<string, unknown>).requireSchemaVersion as number)
                : undefined,
          }
        : null

    const maxPendingJobs =
      typeof params.maxPendingJobs === "number" && Number.isFinite(params.maxPendingJobs)
        ? Math.min(10000, Math.max(1, Math.trunc(params.maxPendingJobs)))
        : 100
      ...
      patch.inputContract =
        params.inputContract && typeof params.inputContract === "object"
          ? {
              requireType:
                typeof (params.inputContract as Record<string, unknown>).requireType === "string"
                  ? (params.inputContract as Record<string, unknown>).requireType
                  : undefined,
              requireSchemaVersion:
                typeof (params.inputContract as Record<string, unknown>).requireSchemaVersion === "number"
                  ? (params.inputContract as Record<string, unknown>).requireSchemaVersion
                  : undefined,
            }
          : null
    }
