...
When connecting with kind="route", provide the route value that triggers this edge.`,
      parameters: {
        type: 'object',
        properties: {
          action: { type: 'string', enum: ['add', 'update', 'delete', 'connect'], description: 'The action to perform' },
          pipelineId: { type: 'string', description: 'The pipeline ID' },
          ...
          instruction: { type: 'string', description: 'Instruction for the agent executing this node (for add/update)' },
          role: { type: 'string', enum: ['planner', 'coder', 'tester', 'reviewer', 'operator'], description: 'Executor role (for add/update)' },
          agentId: { type: 'string', description: 'Agent ID to execute this node (required). Call agent_list first to see available agents.' },
          type: { type: 'string', enum: ['task', 'router'], description: 'Node type (for add, default "task")' },
          routePolicy: { type: 'array', items: { type: 'string' }, description: 'Route values for router nodes, e.g. ["yes", "no"]' },
          dependsOn: { type: 'array', items: { type: 'string' }, description: 'Upstream node IDs (for add, creates dependency edges automatically)' },
          ...
          route: { type: 'string', description: 'Route value for conditional routing from router nodes (e.g. "yes", "no"). Only use when connecting FROM a router node to a branch. Omit for normal dependency edges.' },
        },
        required: ['action', 'pipelineId'],
      },
      annotations: { readOnly: false, destructive: false, requiresConfirmation: true, idempotent: false },
      ...
      async execute(args) {
        ...
        switch (action) {
          case 'add': {
            ...
            if (!agentId?.trim()) return { output: 'agentId is required. Call agent_list first to see available agents, then specify which agent should execute this node.', isError: true }

            const dependsOn = a.dependsOn as string[] | undefined
            if (workflow.nodes.length > 0 && (!dependsOn || dependsOn.length === 0)) {
              return { output: 'dependsOn is required when adding a node to a pipeline that already has nodes. Specify which upstream node(s) this node depends on.', isError: true }
            }
            ...
            if (nodeType === 'router') {
              if (!routePolicyValues || routePolicyValues.length < 2) {
                return { output: 'Router nodes require routePolicy with at least 2 values (e.g. ["yes", "no"]).', isError: true }
              }
            ...
            return {
              output: JSON.stringify({
                ...
                node: { id: node.id, name: node.name, type: node.type, role: node.executor.role, instruction: node.instruction },
          ...
          case 'update': {
            ...
            return {
              output: JSON.stringify({
                ...
                node: { id: updated.id, name: updated.name, type: updated.type, enabled: updated.enabled, role: updated.executor.role, instruction: updated.instruction },
              }, null, 2),
              isError: false,
            }
          }
          ...
          case 'delete': {
            ...
            const nextGroups = (workflow.groups ?? []).map(g => ({
              ...g,
          ...
          case 'connect': {
            ...
            if (!workflow.nodes.some(n => n.id === from)) return { output: `Source node "${from}" not found.`, isError: true }
            if (!workflow.nodes.some(n => n.id === to)) return { output: `Target node "${to}" not found.`, isError: true }
