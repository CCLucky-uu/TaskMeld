  ...
  return [
    {
      ...
      async execute(args) {
        ...
        try {
          ...
          if (result.items.length === 0) return { output: `No artifacts found for pipeline "${pipelineId}".`, isError: false }
          return {
          ...
          return { output: `Failed to list artifacts: ${err instanceof Error ? err.message : String(err)}`, isError: true }
        }
      },
    },
    {
      ...
      async execute(args) {
        ...
        try {
          ...
          return { output: `Failed to read artifact: ${err instanceof Error ? err.message : String(err)}`, isError: true }
        }
      },
    },
  ]
