  ...
  return [
    {
      ...
      async execute() {
        ...
        try {
          ...
          return {
            output: JSON.stringify(list.map(s => ({
              id: s.id,
              title: s.title,
            })), null, 2),
            isError: false,
          ...
          return { output: `Failed to list sessions: ${err instanceof Error ? err.message : String(err)}`, isError: true }
        }
      },
    },
    {
    ...
    },
    {
      ...
      async execute(args) {
        ...
        try {
          ...
          return { output: `Failed to get session history: ${err instanceof Error ? err.message : String(err)}`, isError: true }
        }
      },
    },
  ]
